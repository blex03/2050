...
class CustomSet:
    ...
    def remove(self, item):
        ...
        # update lengths <= 1/2*buckets, and 1/2*buckets >= min_buckets)
        self._len -= 1

        # rehash if necessary (item 
        if item <= 0.5 * self._n_buckets and self._n_buckets >= 2*self._min_buckets:
            self._rehash(self._n_buckets//2)
